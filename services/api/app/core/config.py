...
class Settings(BaseSettings):
    ...
    community_curation_bridge_enabled: bool = Field(
        default=False,
        alias="COMMUNITY_CURATION_BRIDGE_ENABLED",
    )
    ...
    def workflow_actor_tokens(self) -> dict[UUID, str]:
        raw = json.loads(self.workflow_actor_tokens_json or "{}")
    ...
    def supabase_jwt_algorithms(self) -> list[str]:
        algorithms = [item.strip() for item in self.supabase_jwt_algorithms_csv.split(",")]
        algorithms = [algorithm for algorithm in algorithms if algorithm]
        if not algorithms:
            raise ValueError("SUPABASE_JWT_ALGORITHMS must include at least one algorithm")
        return algorithms

    def identity_bootstrap_admin_emails(self) -> set[str]:
        return {
            item.strip().lower()
            for item in self.identity_bootstrap_admin_emails_csv.split(",")
            if item.strip()
        }
