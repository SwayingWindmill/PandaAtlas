...
def _moderation_session() -> Iterator[Session]:
    if not settings.review_moderation_enabled or not settings.moderation_sanctions_enabled:
...
def _ensure_subject(session: Session, account_id: UUID):
...
def _action_row(session: Session, action_id: UUID, *, lock: bool = False):
    lock_sql = " for update of action" if lock else ""
    row = session.execute(
        text(
            f"""
            select action.*,
                   exists(
                     select 1 from review_moderation.effective_sanctions effective
                     where effective.action_id = action.action_id
                   ) as effective
            from review_moderation.moderation_actions action
            where action.action_id = :action_id{lock_sql}
            """
        ),
        {"action_id": action_id},
...
def _action_read(row) -> ModerationActionRead:
...
def _append_action(
    ...
    *,
    ...
    session.execute(
        ...
        {"account_id": account_id, "resulting_version": resulting_version},
    )
    return action_id
...
def get_account_moderation(account_id: UUID) -> AccountModerationRead:
    with _moderation_session() as session:
        ...
        return AccountModerationRead(
            ...
            actions=[_action_read(row) for row in rows],
        )
...
def issue_moderation_action(
    ...
    with _moderation_session() as session:
        replay = session.execute(
            text(
                """
                select action_id, account_id, kind::text
        ...
        if account_id == actor.account_id:
            _audit(
                ...
                account_id=account_id,
                actor_account_id=actor.account_id,
                event_type="moderation.sanction.denied",
                outcome="denied",
                reason="self-sanction-denied",
                correlation_id=correlation_id,
        ...
        subject = _ensure_subject(session, account_id)
        _check_version(subject["version"], command.expected_version)
        ...
        can_manage = actor.has_capability("moderation.sanction.manage")
        if not can_manage:
            if command.kind is not ModerationActionKind.SUBMISSION_RESTRICTED:
...
def restore_moderation_action(
    ...
    with _moderation_session() as session:
        action = _action_row(session, action_id, lock=True)
        ...
        if not bool(action["effective"]):
            ...
            if replay is not None:
                return get_account_moderation(UUID(str(action["account_id"])))
        ...
        if ModerationActionKind(str(action["kind"])) in _BLOCKING_ACTIONS:
            still_blocked = session.execute(
                text(
                    """
                    select exists(
                      select 1 from review_moderation.effective_sanctions
                      where account_id = :account_id
...
def _appeal_read(row) -> AppealCaseRead:
...
def submit_appeal(
    ...
    with _moderation_session() as session:
        action = _action_row(session, command.sanction_action_id)
        ...
        if existing is not None:
            return get_appeal(UUID(str(existing)))
...
def get_appeal(appeal_case_id: UUID) -> AppealCaseRead:
    with _moderation_session() as session:
        row = session.execute(
            text(
                """
...
def list_appeals(state: AppealCaseState | str = "all", limit: int = 100) -> AppealQueueRead:
    ...
    with _moderation_session() as session:
        ...
        return AppealQueueRead(items=[_appeal_read(row) for row in rows], state=state_value)
...
def claim_appeal(
    ...
    with _moderation_session() as session:
        ...
        if row is None:
            _deny(404, "appeal_case_not_found", "Appeal case not found")
        _check_version(row["version"], command.expected_version)
...
def decide_appeal(
    ...
    with _moderation_session() as session:
        ...
        if appeal is None:
            _deny(404, "appeal_case_not_found", "Appeal case not found")
        _check_version(appeal["version"], command.expected_version)
        ...
        sanction = _action_row(session, UUID(str(appeal["sanction_action_id"])), lock=True)
        account_id = UUID(str(appeal["account_id"]))
        subject = _ensure_subject(session, account_id)
        subject_version = int(subject["version"])

        if command.outcome in {AppealDecisionOutcome.MODIFIED, AppealDecisionOutcome.OVERTURNED}:
            if bool(sanction["effective"]):
                _append_action(
                    session,
                    account_id=account_id,
                    kind=ModerationActionKind.RESTORATION,
        ...
        elif str(subject["account_state"]) == AccountState.SUSPENDED.value:
            _set_account_state(
                session,
                account_id=account_id,
                next_state=AccountState.ACTIVE,
        ...
        session.execute(
            text(
                """
                update review_moderation.appeal_cases
                set state = 'closed', outcome = cast(:outcome as review_moderation.appeal_decision_outcome),
        ...
        session.execute(
            ...
            ),
            {
                ...
                "details": json.dumps(
                    {"replacement_action_id": str(replacement_action_id) if replacement_action_id else None},
                    sort_keys=True,
                ),
                "correlation_id": correlation_id,
                "idempotency_key": command.idempotency_key,
