...
def test_current_release_reports_independent_versions_and_licenses() -> None:
    ...
    assert payload["dataset_release_version"] == "2026.07.31.1"
    assert payload["public_schema_version"] == "1.3.0"
...
def test_release_api_responses_carry_current_release_headers() -> None:
    ...
    assert response.status_code == 200
    assert response.headers["X-PandaAtlas-Dataset-Version"] == "2026.07.31.1"
    assert response.headers["X-PandaAtlas-Public-Schema-Version"] == "1.3.0"
...
def test_release_api_serves_the_exact_versioned_panda_snapshot() -> None:
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["release"]["dataset_release_version"] == "2026.07.31.1"
    assert len(payload["records"]) == 39
