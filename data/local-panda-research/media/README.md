...
As of 2026-07-26, the vault contains **313 candidates**. **306 media files** are present locally, totalling **949,990,700 bytes** (about 950.0 MB); seven URLs remain failed. Four are earlier historic remote-asset failures, and three are traceable San Diego Zoo Hua Mei image URLs whose retired host now presents a TLS certificate for a different hostname. TLS verification is not disabled, and the failed candidates remain as replacement leads rather than being removed.
