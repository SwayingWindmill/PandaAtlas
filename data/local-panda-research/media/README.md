...
As of 2026-07-26, the vault contains **397 candidates**. **390 media files** are present locally, totalling **990,198,426 bytes** (about 990.2 MB); seven URLs remain failed. Four are earlier historic remote-asset failures, and three are traceable San Diego Zoo Hua Mei image URLs whose retired host now presents a TLS certificate for a different hostname. TLS verification is not disabled, and the failed candidates remain as replacement leads rather than being removed.
