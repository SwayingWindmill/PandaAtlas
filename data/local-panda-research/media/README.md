...
As of 2026-07-26, the vault contains **355 candidates**. **348 media files** are present locally, totalling **959,096,237 bytes** (about 959.1 MB); seven URLs remain failed. Four are earlier historic remote-asset failures, and three are traceable San Diego Zoo Hua Mei image URLs whose retired host now presents a TLS certificate for a different hostname. TLS verification is not disabled, and the failed candidates remain as replacement leads rather than being removed.
