...
As of 2026-07-25, the vault contains **239 candidates**. **232 media files** are present locally, totalling **921,058,982 bytes** (about 921.1 MB); seven URLs remain failed. Four are earlier historic remote-asset failures, and three are traceable San Diego Zoo Hua Mei image URLs whose retired host now presents a TLS certificate for a different hostname. TLS verification is not disabled, and the failed candidates remain as replacement leads rather than being removed.
