...
As of 2026-07-25, the vault contains **237 candidates**. **230 media files** are present locally, totalling **920,650,275 bytes** (about 920.7 MB); seven URLs remain failed. Four are earlier historic remote-asset failures, and three are traceable San Diego Zoo Hua Mei image URLs whose retired host now presents a TLS certificate for a different hostname. TLS verification is not disabled, and the failed candidates remain as replacement leads rather than being removed.
