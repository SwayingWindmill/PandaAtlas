...
export default async function FanV07Pandas({ params }: Props) {
  const { locale: rawLocale } = await params;
  const locale = parsePublicLocale(rawLocale);
  if (!locale) notFound();
  const zh = locale === "zh";
  const atlas = loadPublishedAtlasDataset(locale);
  const photographed = atlas.data.pandas.filter((panda) => Boolean(panda.cover_image_url));
  const pandas = choosePandas(photographed, preferred, 25);
  const featured = pandas[0] ?? null;
  const other = locale === "zh" ? "en" : "zh";

  return (
    <PrototypeShell locale={locale} active="pandas" alternatePath={`/${other}/prototype/fan-v07/pandas`}>
            ...
            <section className={styles.spotlight}>
              ...
              <div className={styles.spotlightCard}>
                ...
                <div className={styles.spotlightCopy}>
                  ...
                  <Link className={styles.textAction} href={`/${locale}/prototype/fan-v07/panda/${featured.slug}` as Route}>{zh ? "查看熊猫档案" : "View panda profile"}<ArrowRight aria-hidden="true" /></Link>
                </div>
              </div>
            </section>
          ) : null}
          ...
          <section className={styles.directorySection}>
            ...
            <div className={styles.pandaGrid}>
              {pandas.slice(featured ? 1 : 0).map((panda) => (
                <Link className={styles.pandaCard} key={panda.id} href={`/${locale}/prototype/fan-v07/panda/${panda.slug}` as Route}>
