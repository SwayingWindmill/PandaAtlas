...
export default async function FanV07Families({ params }: Props) {
  ...
  return (
    <PrototypeShell locale={locale} active="families" alternatePath={`/${other}/prototype/fan-v07/families`}>
      <main className={styles.subPage}>
          ...
          {featuredStory && featuredCopy ? (
            <section className={styles.featureStory}>
              ...
              <div className={styles.featureStoryCopy}>
                <small>{zh ? "Featured family story" : "Featured family story"}</small>
                <h2>{featuredCopy.title}</h2>
                <p>{featuredCopy.summary}</p>
                <div className={styles.portraitStack}>
                  {featuredMembers.slice(0, 7).map((member) => (
                    <Link key={member.id} href={`/${locale}/prototype/fan-v07/panda/${member.slug}` as Route} title={pandaName(member, locale)}>
              ...
              return (
                <article className={styles.familyStoryCard} key={story.id}>
                  ...
                  <div className={styles.familyStoryText}>
                    ...
                    <div className={styles.miniMembers}>
                      {members.slice(0, 7).map((member) => (
                        <Link key={member.id} href={`/${locale}/prototype/fan-v07/panda/${member.slug}` as Route}>
                  ...
                  <div className={styles.familyStoryActions}>
                    ...
                    {members[0] ? <Link href={`/${locale}/prototype/fan-v07/lineage?focus=${members[0].slug}` as Route}><GitFork aria-hidden="true" />{zh ? "谱系" : "Lineage"}</Link> : null}
                  </div>
                </article>
              );
            })}
        ...
        <section className={styles.lineageCallout}>
          <div className={`${styles.subShell} ${styles.lineageCalloutInner}`}>
            ...
            <Link className={styles.textAction} href={`/${locale}/prototype/fan-v07/lineage` as Route}>{zh ? "打开互动谱系图" : "Open interactive lineage"}<ArrowRight aria-hidden="true" /></Link>
          </div>
        </section>
      </main>
    </PrototypeShell>
