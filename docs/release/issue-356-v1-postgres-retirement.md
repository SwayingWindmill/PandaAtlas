...
The post-DDL async smoke also exposed one pre-existing `publication.release.activated` Outbox event from the #333 cutover with zero publish attempts and no error. Production had no scheduler secret and therefore no operational scheduler path for `GET /internal/jobs/async-downstream`. The API project is on Vercel Hobby, whose daily-only Cron limit rejects the required five-minute pump, so #356 closes the gap with Supabase Cron instead of silently degrading cadence. Migration `0052_async_downstream_scheduler.sql` enables `pg_cron`/`pg_net`, schedules the same bounded NestJS endpoint every five minutes, and reads its environment-specific endpoint URL and bearer secret from Supabase Vault. Environments without those Vault entries intentionally no-op rather than calling production.
