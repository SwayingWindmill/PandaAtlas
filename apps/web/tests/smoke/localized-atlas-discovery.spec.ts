...
test("restores URL-driven Atlas pagination through browser history", async ({ page }) => {
  ...
  await expect(next).toHaveAttribute("href", "/zh/pandas?page=2");

  await next.click();
  await expect(page).toHaveURL(/\/zh\/pandas\?page=2$/);
  await expect(page.getByTestId("atlas-result-list").getByRole("listitem")).toHaveCount(4);

  await page.goBack();
  await expect(page).toHaveURL(/\/zh\/pandas$/);
  await expect(page.getByTestId("atlas-result-list").getByRole("listitem")).toHaveCount(4);
});

test("combines reviewed search, sex, facility and sort state", async ({ page }) => {
  await page.goto(`/en/pandas?q=bao&sex=male&facility=${SMITHSONIAN_FACILITY_ID}&sort=name`);

  const results = page.getByTestId("atlas-result-list");
  await expect(results.getByRole("listitem")).toHaveCount(1);
  await expect(results.getByRole("link", { name: /Bao Li/ })).toHaveAttribute("href", "/en/pandas/bao-li");
...
test("normalizes unsupported and invalid URL state instead of silently retaining it", async ({ page }) => {
  ...
  await expect(page.getByTestId("atlas-result-summary")).toContainText("679 pandas are currently included");
});

test("preserves compatible discovery state across the locale switch", async ({ page }) => {
  await page.goto("/zh/pandas?q=bao&sex=male&sort=name");

  await expect(page.getByRole("link", { name: "English", exact: true })).toHaveAttribute(
    "href",
    "/en/pandas?q=bao&sex=male&sort=name",
  );
});
