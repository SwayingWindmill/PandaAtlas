...
test("serves canonical bilingual Passport routes", async ({ page, request }) => {
  ...
  await expect(page.getByRole("link", { name: "使用邮箱验证码登录" })).toHaveAttribute("href", "/auth/login?next=/zh/my-pandas");
...
test("deletes legacy anonymous saves without converting them", async ({ page }) => {
  ...
  expect(stored.preferences.saved).toEqual([]);
...
test("signed-out Follow creates Pending Intent before OTP", async ({ page }) => {
  ...
  await follow.click();
  await expect(page).toHaveURL(/\/auth\/login\?next=%2Fen%2Fatlas%2Fmei-xiang$/);
  expect(requestBody).toMatchObject({ locale: "en", return_path: "/en/atlas/mei-xiang" });
});
