...
export async function proxyAdminGamesRequest(
  request: NextRequest,
  pathSegments: string[],
  ...
  const target = `${FASTAPI_BASE_URL}/api/v1/admin/games${path ? `/${path}` : ""}${request.nextUrl.search}`;
  const headers = new Headers({
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { detail: "Invalid Admin games response" },
        { status: 502, headers: responseHeaders },
      );
    }
  } catch {
    return NextResponse.json(
      { detail: "Admin games service unavailable" },
