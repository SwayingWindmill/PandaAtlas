...
export async function proxyAdminMediaRequest(
  request: NextRequest,
  pathSegments: string[],
  ...
  const target = `${FASTAPI_BASE_URL}/api/v1/admin/media${path ? `/${path}` : ""}${request.nextUrl.search}`;
  const headers = new Headers({
  ...
  const uploadReference = request.headers.get("x-upload-reference");
  if (uploadReference) headers.set("X-Upload-Reference", uploadReference);
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { detail: "Invalid Admin media response" },
        { status: 502, headers: responseHeaders },
      );
    }
  } catch {
    return NextResponse.json(
      { detail: "Admin media service unavailable" },
