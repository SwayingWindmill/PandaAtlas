...
export async function proxyAdminArchiveRequest(
  ...
  const target = `${FASTAPI_BASE_URL}/api/v1/admin/archive${path ? `/${path}` : ""}${request.nextUrl.search}`;
  const headers = new Headers({
  ...
  try {
    ...
    return NextResponse.json(
      { detail: "Archive administration service unavailable" },
  ...
  try {
    ...
    return NextResponse.json(
      { detail: "Invalid Archive administration response" },
