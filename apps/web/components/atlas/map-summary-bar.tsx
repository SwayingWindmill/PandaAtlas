      ...
      <div className="grid w-full max-w-[760px] divide-y divide-[rgba(63,125,72,0.1)] border border-[rgba(63,125,72,0.12)] bg-[rgba(247,246,242,0.88)] backdrop-blur-sm md:grid-cols-3 md:divide-x md:divide-y-0">
        {items.map((item) => (
          <div key={item.label} className="px-5 py-3">
            <div className="flex items-center gap-2 text-[12px] font-medium leading-5 text-[var(--atlas-muted-fg)]">
            ...
            <div className="mt-2 flex items-end justify-between gap-4">
              ...
              <p className="max-w-[12rem] text-right text-[12px] leading-5 text-[var(--atlas-muted-fg)]">{item.hint}</p>
            </div>
          </div>
        ))}
      </div>
