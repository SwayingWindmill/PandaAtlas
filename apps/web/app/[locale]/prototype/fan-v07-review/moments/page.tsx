...
import { withReviewVisual } from "../review-data";
...
export default async function FanV07ReviewMoments({ params, searchParams }: Props) {
  ...
  const feature = moments.find((item) => item.participants.some((panda) => withReviewVisual(panda).cover_image_url)) ?? moments[0] ?? null;
  const featurePandaRaw = feature?.participants.find((panda) => withReviewVisual(panda).cover_image_url) ?? feature?.participants[0] ?? null;
  const featurePanda = featurePandaRaw ? withReviewVisual(featurePandaRaw) : null;
        ...
        <div className={styles.subShell}>
          ...
          <nav className={styles.monthRail} aria-label={zh ? "月份" : "Months"}>
            ...
            {monthCounts.map((item) => (
              <Link key={item.month} href={reviewHref(locale, year, item.month) as Route} aria-current={month === item.month ? "page" : undefined} data-empty={item.count === 0 ? "true" : undefined}>
          ...
          {feature && featurePanda ? (
            <section className={styles.momentSpotlight}>
              ...
              <div className={styles.momentSpotlightCopy}>
                ...
                <Link className={styles.textAction} href={`/${locale}/pandas/${featurePanda.slug}` as Route}>{zh ? "查看熊猫档案" : "View panda profile"}<ArrowRight aria-hidden="true" /></Link>
              </div>
            </section>
          ) : null}
                ...
                {moments.map((moment) => {
                  ...
                  return (
                    <article className={styles.momentRow} key={moment.id}>
                      ...
                      {participant ? <Link href={`/${locale}/pandas/${participant.slug}` as Route} aria-label={pandaName(participant, locale)}><ArrowRight aria-hidden="true" /></Link> : null}
                    </article>
                  );
                })}
              </div>
