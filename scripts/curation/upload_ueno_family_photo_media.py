...
def load_uploads() -> list[Upload]:
    ...
    if len(uploads) != EXPECTED_DERIVATIVES:
        raise ValueError(
            f"Expected {EXPECTED_DERIVATIVES} reviewed derivatives, found {len(uploads)}"
        )
    return uploads
...
def verify_remote_uploads(uploads: list[Upload]) -> None:
    with tempfile.TemporaryDirectory(prefix="panda-atlas-ueno-r2-verify-") as temporary:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify and upload the reviewed Ueno family WebP derivatives to R2."
    )
    parser.add_argument(
        "--execute",
        action="store_true",
