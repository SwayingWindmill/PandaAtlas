...
if (!existsSync(buildIdPath) || !existsSync(manifestPath)) {
  console.error(`[notification-center-budget] ${nextRoot} is not a completed production build`);
  process.exit(1);
}

const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
const routeEntries = ["/layout", "/[locale]/layout", "/[locale]/me/inbox/page"];
const missingEntries = routeEntries.filter((entry) => !manifest.pages?.[entry]);
if (missingEntries.length) {
  console.error(`[notification-center-budget] missing route entries: ${missingEntries.join(", ")}`);
  process.exit(1);
}
...
const measurements = files.map((file) => {
  ...
  if (!existsSync(absolutePath)) throw new Error(`Missing emitted notification asset: ${file}`);
...
const htmlMeasurements = ["en", "zh"].flatMap((locale) => {
  const file = path.join("server", "app", locale, "me", "inbox.html");
  const absolutePath = path.join(nextRoot, file);
...
const report = {
  status: firstLoadJavaScriptBytes <= firstLoadLimitBytes && estimatedInitialTransferBytes <= transferLimitBytes
    ? "PASS"
    : "FAIL",
  first_load_javascript: { bytes: firstLoadJavaScriptBytes, limit_bytes: firstLoadLimitBytes },
