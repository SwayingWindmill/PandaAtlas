...
if (!existsSync(buildIdPath)) {
  console.error(`[editorial-home-budget] ${nextRoot} is not a completed production build`);
  process.exit(1);
}
if (!existsSync(manifestPath)) {
  console.error(`[editorial-home-budget] missing build manifest: ${manifestPath}`);
  process.exit(1);
}

const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
const routeEntries = ["/layout", "/[locale]/layout", "/[locale]/page"];
const missingEntries = routeEntries.filter((entry) => !manifest.pages?.[entry]);
if (missingEntries.length) {
  console.error(`[editorial-home-budget] missing route entries: ${missingEntries.join(", ")}`);
  process.exit(1);
}
...
const measurements = files.map((file) => {
  ...
  if (!existsSync(absolutePath)) throw new Error(`Missing emitted Home asset: ${file}`);
...
const htmlMeasurements = ["en", "zh"].flatMap((locale) => {
  const file = path.join("server", "app", `${locale}.html`);
  const absolutePath = path.join(nextRoot, file);
